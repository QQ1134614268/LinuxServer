...
RUN pip install -r /root/world-api/requirements.txt -i https://mirrors.aliyun.com/pypi/simple/
...
# 更新数据库
RUN cd /root/world-api/src/ && /usr/local/bin/python/bin/flask db upgrade
EXPOSE 9090
# CMD python -m cd /root/world-api/src/app.py # bug no module named app todo
CMD cd /root/world-api/src/ && python app.py

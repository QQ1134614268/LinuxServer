...
if [[ $ship_new_version == $ship_old_version && $build_mode == '自动' ]]
then
  echo '没有更新,退出编译'
  exit 0
fi
...
then
  echo 'xc-common,xc-api模块版本已更新, 请先编译依赖模块'
  exit 1
fi
...
cd  $project_dir
java -jar -server -Xms${mem_min}m -Xmx${mem_max}m -Djava.security.egd=file:/dev/./urandom -DprojectName=$project_name -Dname=$project_name -Duser.timezone=Asia/Shanghai  $project_dir/$project_jar > $log_file 2>&1 &
